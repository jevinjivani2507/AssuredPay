...
contract AssuredPay is ChainlinkClient,ConfirmedOwner {
    ...
    constructor(address _vendor, address _customer, uint256 _amount) ConfirmedOwner(_vendor) {
    ...
    function requestDeliveryStatus(bool _mock) public onlyOwner returns (bytes32 requestId) {

        if( keccak256(bytes(order.orderId)) == keccak256("") || keccak256(bytes(order.provider)) == keccak256("")){
    ...
    function fulfill(bytes32 _requestId,  string memory _orderStatus) public recordChainlinkFulfillment(_requestId) {
        result = _orderStatus;
    ...
    function withdrawAmount() external {
        ...
        if(msg.sender == i_customer){
            ...
            if(!s_isTransactionFulfilled && isOrderStatusUpdated && isSufficientBalance){
    ...
    function _check(string memory string1 , string memory string2) internal pure returns (bool) {
